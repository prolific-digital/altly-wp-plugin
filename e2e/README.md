...
- **`instant.spec.js`** — Bulk Generate in Instant mode enqueues to the API, the
  instant worker (`/v2/queue/process`) generates alt text, writes it to Supabase
  and back to the WordPress attachment, and spends exactly **3** credits.
- **`relaxed.spec.js`** — same in Relaxed mode via the Claude Batch path
  (`/v2/batch/submit` + `/v2/batch/poll`), spending exactly **1** credit.
